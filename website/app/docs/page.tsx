  ...
  return (
    <>
      ...
      <section className="docs-layout section-shell">
        ...
        <div className="docs-groups">{docsGroups.map(group => <article className="docs-card" key={group.title}><span className="eyebrow">Guide</span><h2>{group.title}</h2>{group.links.map(link => <Link href={`${repositoryUrl}#${link.toLowerCase().replaceAll(" ", "-")}`} key={link}>{link}</Link>)}</article>)}</div>
      </section>
