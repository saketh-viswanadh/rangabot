...
test("UI wiring is completed-turn only, same-origin, accessible, and content-free", () => {
  ...
  assert.match(component, /<legend className="visually-hidden">Was this response helpful\?<\/legend>/);
  assert.match(component, /aria-label="Helpful"/);
  assert.match(component, /aria-label="Needs improvement"/);
  ...
  assert.match(css, /\.response-feedback-options button \{[^}]*width: 28px;[^}]*height: 28px/);
  assert.match(css, /\.response-feedback-options \{[^}]*flex-wrap: nowrap/);
  assert.match(css, /button\.selected \{[^}]*border-width: 0/);
  assert.doesNotMatch(css, /\.response-feedback-check/);
});
