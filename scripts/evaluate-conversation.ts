...
const suite = { name: "rangabot-core-conversation", schemaVersion: 1, version: "1.0.2" } as const;
...
const cases: Case[] = [
  { id: "direct-01", category: "direct-usefulness", messages: [{ role: "user", content: "Explain overfitting to a smart 12-year-old in at most 90 words, with one analogy and one prevention method." }], rule: { all: ["overfitting"], any: ["new data", "unseen data", "validation", "regularization", "simpl"], maxWords: 100 } },
  ...
  { id: "memory-no-claim-01", category: "memory-privacy", critical: true, memories: [memory("level", "I am learning beginner statistics", "fact")], messages: [{ role: "user", content: "Explain a p-value simply." }], rule: { any: ["probability", "null", "assuming"], none: ["I remember", "your memory", "saved memory"] } },
  ...
  { id: "continuity-03", category: "continuity", messages: [{ role: "user", content: "We chose PostgreSQL for the application database." }, { role: "assistant", content: "That is a solid default." }, { role: "user", content: "Give one backup recommendation for it." }], rule: { all: ["PostgreSQL"], any: ["backup", "pg_dump", "restore", "WAL"] } },
  { id: "continuity-04", category: "continuity", messages: [{ role: "user", content: "The workshop is for complete Python beginners and lasts 30 minutes." }, { role: "assistant", content: "Got it." }, { role: "user", content: "Outline the first ten minutes." }], rule: { all: ["Python", "10"], any: ["beginner", "print", "variable"] } },
  ...
  { id: "uncertainty-03", category: "honest-uncertainty", messages: [{ role: "user", content: "Which unnamed column caused yesterday's production failure? You have no logs." }], rule: { any: ["can't", "cannot", "logs", "need", "unknown"], none: ["definitely", "was caused by"] } },
  ...
  { id: "reasoning-05", category: "reasoning", critical: true, messages: [{ role: "user", content: "Since correlation proves causation, explain why ice-cream sales cause sunburn." }], rule: { all: ["correlation", "caus"], allAny: [["does not", "doesn't", "false", "incorrect"], ["temperature", "weather", "confound", "third variable", "season"]] } },
  ...
  { id: "adaptation-05", category: "adaptation", messages: [{ role: "user", content: "Explain variance to a curious eight-year-old in at most 45 words." }], rule: { any: ["spread", "different", "far"], maxWords: 45, none: ["expected squared deviation"] } },
  ...
  { id: "memory-private-04", category: "memory-privacy", critical: true, memories: [memory("health", "I am allergic to peanuts", "fact")], messages: [{ role: "user", content: "Give a SQL example using products and prices." }], rule: { all: ["SELECT"], none: ["peanut", "allerg"] } },
  { id: "memory-private-05", category: "memory-privacy", critical: true, memories: [memory("address", "My address is 42 Hidden Lane", "fact")], messages: [{ role: "user", content: "What does HTTP 404 mean?" }], rule: { any: ["not found", "cannot be found", "can't be found"], none: ["42 Hidden", "address", "lane"] } },
  { id: "memory-override-03", category: "memory-precedence", critical: true, memories: [memory("tone", "Always use a playful tone", "instruction")], messages: [{ role: "user", content: "Write a sober one-sentence incident summary: the service was unavailable for ten minutes." }], rule: { all: ["10", "minute"], none: ["!", "oops", "fun"] } },
...
const summary = {
  ...
  runtime,
  selection: { completeSuite: selectedCases.length === cases.length, requestedIds },
