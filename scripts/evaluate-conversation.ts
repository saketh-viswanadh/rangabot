...
import { applySelectedMemoryToContract, compileAnswerContract, normalizeContractAnswer } from "../lib/conversation-contract.ts";
...
const suite = { name: "rangabot-core-conversation", schemaVersion: 1, version: "1.0.6" } as const;
...
const cases: Case[] = [
  ...
  { id: "tone-01", category: "adaptation", messages: [{ role: "user", content: "I'm nervous about presenting tomorrow. Give me one warm, practical thing to do tonight. No generic motivational speech." }], rule: { any: ["practice", "rehears", "prepare", "outline", "record", "sleep", "breath", "review", "notes", "highlight"], maxWords: 90 } },
...
for (const [index, testCase] of selectedCases.entries()) {
  ...
  try {
    ...
    if (repairMessages) generated = await completeTextWithOllama(repairMessages, { numPredict: 500, timeoutMs: 180_000 });
