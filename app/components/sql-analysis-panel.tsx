...
type Dataset = { id: string; name: string; path: string; format: "csv" | "parquet"; sizeBytes: number; addedAt: string };
...
export function SqlAnalysisPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="knowledge-backdrop" onMouseDown={onClose}><aside className="sql-panel" role="dialog" aria-modal="true" aria-labelledby="sql-panel-title" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="sql-panel-content"><section className="sql-datasets"><header><div><strong>Approved data</strong><small>CSV or Parquet · 100 MB maximum</small></div><span>{datasets.length}</span></header>
      ...
      <form className="sql-allow-form" onSubmit={approve}><input value={datasetPath} onChange={(event) => setDatasetPath(event.target.value)} placeholder="/absolute/path/to/data.csv" aria-label="Dataset path" maxLength={1024} /><button type="submit" disabled={busy || !datasetPath.trim()}>Allow</button></form><p>Approval stores only the local path. Revoking never deletes the file.</p>
