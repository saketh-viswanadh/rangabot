...
export function ProfileManager({ onSwitchingChange, onActiveProfileChange, onRecoveryRequiredChange }: {
  ...
  return <>
    ...
    {open && createPortal(<div className="profile-backdrop app-shell" data-appearance={applicationShell?.dataset.appearance} data-palette={applicationShell?.dataset.palette} onMouseDown={() => { if (!busy) closeProfiles(); }}>
      <section ref={dialogRef} className="profile-dialog" data-mode={mode} role="dialog" aria-modal="true" aria-labelledby="profile-title" aria-busy={busy} onMouseDown={(event) => event.stopPropagation()}>
        <header className="profile-dialog-header"><div><span>Profiles</span><h2 id="profile-title">{mode === "choose" ? "Choose a profile" : mode === "create" ? "Create a profile" : "Manage profiles"}</h2><p>{mode === "choose" ? "Each profile keeps its chats, memory, and settings separate." : `Active: ${marker}`}</p></div><button ref={closeRef} type="button" onClick={closeProfiles} disabled={busy} aria-label="Close Profiles"><CraftIcon name="close" /></button></header>
