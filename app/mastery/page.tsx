...
export default function MasteryPage() {
  ...
  return (
    <main className="mastery-shell">
      ...
      <section className="mastery-paths" aria-label="Rangabot mastery paths">
        {tree.branches.map((branch, branchIndex) => (
          <article className="mastery-branch" key={branch.id} style={{ "--branch-index": branchIndex } as CSSProperties}>
            <header><span className="branch-glyph"><CraftIcon name={branchIcons[branch.id] ?? "mastery"} /></span><div><small>Mastery path</small><h2>{branch.name}</h2></div><strong>{branch.score.toFixed(1)}<small>/5 weighted</small></strong></header>
            <p>{branch.summary}</p>
            <ol>{branch.nodes.map((node, nodeIndex) => <li key={node.id} data-status={node.status} style={{ "--node-index": nodeIndex } as CSSProperties}>
              <button type="button" onClick={() => openSelected(node, branch.name)} aria-label={`${node.name}, ${statusLabels[node.status]}, weighted progress ${node.score} out of 5`}>
                <span className="node-emblem"><i /></span>
                <span className="node-copy"><strong>{node.name}</strong><small>{statusLabels[node.status]} · {node.score.toFixed(1)}/5 weighted</small></span>
              </button>
      ...
      <section className="mastery-achievements" aria-labelledby="achievement-title"><div><span className="mastery-kicker">Built by the pack</span><h2 id="achievement-title">Achievements remembered</h2><p>Every credited milestone stays attached to the people who helped unlock it. Claims use merged evidence and an owner-controlled approval event bound to the exact current head. A 2026-08-10 settings audit observed the trusted metadata-only check as required on main; current enforcement must still be reverified because GitHub settings are mutable. CODEOWNERS requests review without requiring approval.</p></div><ul>{contributorsData.contributors.map((contributor) => <li key={contributor.github}><a href={`https://github.com/${contributor.github}`} target="_blank" rel="noreferrer"><span className="contributor-avatar" aria-hidden="true">{contributor.avatar ? <Image src={contributor.avatar} alt="" width={48} height={48} unoptimized /> : contributor.name.slice(0, 1)}</span><span><strong>{contributor.name}</strong><small>@{contributor.github} · {contributor.role}</small><em>{contributor.achievement}</em><b>{contributor.claims.length} official evidence-linked node claims</b><span className="contributor-claims">{contributor.claims.map((claim) => <i key={claim.nodeId}>{nodeNames.get(claim.nodeId)}</i>)}</span></span></a></li>)}</ul><a className="claim-skill" href={`${repositoryUrl}/issues/new?template=mastery-claim.yml`} target="_blank" rel="noreferrer">Request an official contribution claim <CraftIcon name="external" size={12} /></a></section>
      ...
      {selected && <div className="mastery-modal" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) closeSelected(); }}>
        <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="selected-mastery-title" aria-describedby="selected-mastery-description" data-status={selected.status}>
          ...
          <div className="detail-score"><strong>{selected.score.toFixed(1)}</strong><span>/ 5 weighted progress</span></div><p id="selected-mastery-description">{selected.description}</p>
