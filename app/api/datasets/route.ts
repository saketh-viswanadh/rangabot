...
export function GET() {
  try { return NextResponse.json({ datasets: listApprovedDatasets().map(browserDataset) }); }
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json({ dataset: browserDataset(approveDataset(body.path)) }, { status: 201 }); }
