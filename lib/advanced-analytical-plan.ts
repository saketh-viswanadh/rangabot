...
type Operation = "ratio" | "conditional_rate" | "duration_average" | "threshold_count" | "period_growth" | "per_entity_average" | "anti_join";
type Operator = "eq" | "neq" | "lt" | "lte" | "gt" | "gte" | "is_null" | "is_not_null";
type Filter = { column: string; operator: Operator; value: string };
...
const operations: Operation[] = ["ratio", "conditional_rate", "duration_average", "threshold_count", "period_growth", "per_entity_average", "anti_join"];
...
const baseSchema = {
  ...
  required: ["action", "operation", "source", "metric", "secondaryMetric", "entity", "dimensions", "startField", "endField", "dateField", "relatedField", "filters", "numeratorFilters", "denominatorFilters", "threshold", "decimals", "firstStart", "firstEnd", "secondStart", "secondEnd", "explanation"],
  properties: {
    action: { type: "string" }, operation: { type: "string" }, source: { type: "string" }, metric: { type: "string" }, secondaryMetric: { type: "string" }, entity: { type: "string" }, dimensions: { type: "array", items: { type: "string" } },
...
export function buildAdvancedAnalyticalSchema(messages: ChatMessage[], dataset: ApprovedDataset, columns: DatasetColumn[]) {
  ...
  for (const name of ["metric", "secondaryMetric", "entity", "startField", "endField", "dateField", "relatedField"]) properties[name].enum = ["", ...fields];
...
export function buildAdvancedAnalyticalMessages(messages: ChatMessage[], dataset: ApprovedDataset, columns: DatasetColumn[]): ChatMessage[] {
  ...
  return [
    { role: "system", content: "You are a domain-neutral local analytical planner, not a SQL writer. Map the request to exactly one generic operation using only enum-approved schema fields. ratio divides an aggregate metric by a secondary aggregate metric. conditional_rate divides rows matching numeratorFilters by rows matching denominatorFilters. duration_average averages the elapsed time between startField and endField. threshold_count counts entities whose grouped row count reaches threshold. period_growth compares summed metric over two explicit date ranges. per_entity_average sums metric per entity then averages those sums. anti_join returns source entities with no relatedField match. If the operation or required fields are not representable, return clarify or unavailable. Never infer causal effects, forecasts, or unsupported statistics." },
    { role: "user", content: `REQUEST:\n${request}\n\nSCHEMA:\n${schema}\n\nReturn every required JSON field. Copy table.column values exactly. Empty unused fields and arrays are required. Filter values must come from the request. Dates use YYYY-MM-DD with half-open ranges. Explain the calculation or exact boundary without claiming a result.` },
  ];
}

export function shouldUseAdvancedAnalyticalPlan(request: string) {
  return /\b(?:divided by|ratio|percentage|average (?:duration|elapsed|.+ time)|time between|how long|at least \d+|growth (?:from|between)|average .+ per |never \w+|without (?:a |any )?match)\b/i.test(request);
}
...
export function parseAdvancedAnalyticalPlan(raw: string): AdvancedAnalyticalPlan {
  ...
  if (!(["query", "clarify", "unavailable"] as unknown[]).includes(item.action) || !operations.includes(item.operation as Operation)
    || typeof item.source !== "string" || typeof item.metric !== "string" || typeof item.secondaryMetric !== "string" || typeof item.entity !== "string" || !Array.isArray(item.dimensions)
...
function words(value: string) {
  return value.toLowerCase().replaceAll("_", " ").match(/[a-z0-9]+/g)?.map((word) => word.length > 4 && word.endsWith("s") ? word.slice(0, -1) : word) ?? [];
}
...
export function auditAdvancedAnalyticalPlan(plan: AdvancedAnalyticalPlan, request: string, columns: DatasetColumn[]): AnalyticalPlanAudit {
  ...
  const cleanFilters = (filters: Filter[], field: string) => filters.filter((filter) => {
    ...
    const supported = boolean !== undefined || (filter.value.trim() !== "" && requestContainsLiteral(request, filter.value) && fieldEvidence(filter.column, request) > 0);
...
export function compileAdvancedAnalyticalPlan(plan: AdvancedAnalyticalPlan, columns: DatasetColumn[]): SqlProposal {
  ...
  const usedValues = [plan.metric, plan.secondaryMetric, plan.entity, ...plan.dimensions, plan.startField, plan.endField, plan.dateField, plan.relatedField, ...plan.filters.map((f) => f.column), ...plan.numeratorFilters.map((f) => f.column), ...plan.denominatorFilters.map((f) => f.column)].filter(Boolean);
