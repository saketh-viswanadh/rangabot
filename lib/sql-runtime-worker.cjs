...
function boundedSchema(rows, hasTables, constraints = []) {
  ...
  if (constraints.length > MAX_SCHEMA_COLUMNS * 2) throw resourceLimit("The approved dataset has too many relational constraints.");
  const primaryKeys = new Set();
  const references = new Map();
  for (const row of constraints) {
    const table = String(row[0]);
    const kind = String(row[1]);
    const columns = Array.isArray(row[2]) ? row[2].map(String) : [];
    if (kind === "PRIMARY KEY") for (const column of columns) primaryKeys.add(`${table}\0${column}`);
    if (kind !== "FOREIGN KEY") continue;
    const referencedTable = typeof row[3] === "string" ? row[3] : "";
    const referencedColumns = Array.isArray(row[4]) ? row[4].map(String) : [];
    for (const identifier of [table, referencedTable, ...columns, ...referencedColumns]) {
      if (Buffer.byteLength(identifier, "utf8") > MAX_COLUMN_NAME_BYTES) throw resourceLimit("A schema relationship identifier exceeded the safe length limit.");
    }
    if (!referencedTable || referencedColumns.length !== columns.length) throw new Error("The dataset contains an invalid foreign-key relationship.");
    columns.forEach((column, index) => {
      const key = `${table}\0${column}`;
      references.set(key, [...(references.get(key) ?? []), { table: referencedTable, column: referencedColumns[index] }]);
    });
  }
  const schema = rows.map((row) => {
    if (!hasTables) return { name: String(row[0]), type: String(row[1]) };
    const table = String(row[0]);
  ...
  for (const column of schema) {
    for (const [field, value] of Object.entries(column)) {
      ...
      if (serialized === undefined) throw resourceLimit("The dataset schema contained an invalid value.");
      bytes = addBytes(bytes, serializedStringBytes(field, MAX_COLUMN_NAME_BYTES, message) + Buffer.byteLength(serialized, "utf8") + 4, MAX_SCHEMA_BYTES, "The dataset schema exceeded the 256 KB transfer limit.");
    }
  }
  return schema;
}
...
async function inspectSchema(request) {
  ...
  try {
    ...
    `);
    const constraintResult = await connection.runAndReadAll(`
    ...
    `);
    return boundedSchema(result.getRows(), true, constraintResult.getRowsJson());
  } finally {
