...
export type ApprovedDataset = { id: string; name: string; path: string; format: "csv" | "parquet"; sizeBytes: number; addedAt: string };
...
function readRegistry(): ApprovedDataset[] {
  ...
  if (!Array.isArray(value) || !value.every((item) => item && typeof item === "object"
    ...
    && typeof (item as ApprovedDataset).path === "string" && ((item as ApprovedDataset).format === "csv" || (item as ApprovedDataset).format === "parquet")
...
export function approveDataset(inputPath: string): ApprovedDataset {
  if (!inputPath.trim() || inputPath.length > 1024 || !isAbsolute(inputPath)) throw new Error("Enter an absolute CSV or Parquet file path.");
  ...
  const dataset: ApprovedDataset = {
    ...
    format: validated.extension === ".csv" ? "csv" : "parquet", sizeBytes: validated.sizeBytes, addedAt: new Date().toISOString(),
  };
